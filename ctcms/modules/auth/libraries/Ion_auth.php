...
class Ion_auth {
    ...
    public function __construct() {
        ...
        if (get_cookie('sma_language')) {
            if (file_exists('ctcms/language/' . get_cookie('sma_language') . '/sma_lang.php') && is_dir('ctcms/language/' . get_cookie('sma_language'))) {
        ...
        if ($this->logged_in()) {
            ...
            define("ENABLE_LOGGING", (boolean) $SETTING->enable_log);
            define("ENABLE_MSG", (boolean) $SETTING->enable_msg);
            define("ENABLE_LOGGING_DATA", ($SETTING->log_items[0]=='1'?TRUE:FALSE));
            define("ENABLE_LOGGING_USER",  ($SETTING->log_items[1]=='1'?TRUE:FALSE));
            define("ENABLE_LOGGING_MSG",  ($SETTING->log_items[2]=='1'?TRUE:FALSE));
            define("SERVER_CHK_INTERVAL",($SETTING->serverchk_interval));
            define("USER_ACTIVE_INTERVAL", 10000);
    ...
    public function logout() {
        ...
        $this->ion_auth_model->deactivate($this->session->userdata['user_id']);
        $identity = $this->config->item('identity', 'ion_auth');
    ...
    public function getUserWarehouse($id) {
        ...
        $q = $this->db->get_where('warehouses_users', array('user_id' => $id));
        if (count($q)) {
            return $q->row();
        }
        return FALSE;
